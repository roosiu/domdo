  ...
  $(function () {
    $.get("wydruki_karty_wyposazenia_podglad.htm", function (data) {
      ...
      if ($_POST) {
        ...
        if($typ == "odzież"){
          $zapytanie_i = 'SELECT * FROM karty_odziezowe WHERE pracownik="'.$pracownik.'" ORDER BY id ASC';
          /////echo 'jQuery("#pole_zakonczone").html(" - zmiany niezakończone");';
        }else if($typ == "wyposażenie 50-200"){
          $zapytanie_i = 'SELECT * FROM inwentarz WHERE wartosc_ewid < 200 AND przypisane_do="'.$pracownik.'" ORDER BY id ASC';
          /////echo 'jQuery("#pole_zakonczone").html(" - zmiany zakończone");';
        }else if($typ == "wyposażenie 200-2000"){
          $zapytanie_i = 'SELECT * FROM inwentarz WHERE wartosc_ewid > 200 AND wartosc_ewid < 2000 AND przypisane_do="'.$pracownik.'" ORDER BY id ASC';
