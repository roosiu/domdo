...
deleteRecord(($(this).attr("value")),"nieruchomosci_lokalowe");
...
if ($user->check()) { // Tylko dla użytkowników zalogowanych
    ...
    <div class="jumbotron mb-n4 bg-white">
            ...
            <form method="post" action="nieruchomosci_lokalowe.php"><i class="fa fa-fort-awesome" aria-hidden="true"></i> <b>NIERUCHOMOŚCI LOKALOWE</b> <br/><span class="small" id="naglowek_dziennik"></span><span class="small" id="naglowek_dziennik2"></span>
            </div>
            <div class="col">
...
echo '</div></form>';
                        ...
                        echo '<table id="tabela_gl" class="table table-sm table-striped table-hover">
                              ...
                              <tr class="text-center">
                                ...
                                <th scope="col">Indeks</th>
                                  ...
                                  <th scope="col">Rodzaj działalności</th>
                                  <th scope="col">Najemca</th>
                                  <th scope="col">Opiekun</th>
                                  <th scope="col">Właściciel</th>
                                  <th scope="col">Kontakt</th>
                                  <th width="col" scope="col">Dod. info</th>
                                  <th class="no-sort dontprint"></th>
                                  <th scope="col" class="text-center dontprint no-sort"><a href="nieruchomosci_lokalowe_edytuj.php" role="button" class="btn btn-dark btn-sm text-uppercase"><i class="fa fa-plus" aria-hidden="true"></i> Dodaj<br/>pozycję</a>
                                  </th>
                                </tr>
                                </thead>
                              <tbody>
                ...
                if($adres_ulica){
                $ulica_i = ' AND lokalizacja = "'.pojed_zapyt('SELECT nazwa FROM ulice WHERE `id` ='.$adres_ulica).'"';

                }

                if($adres_nrbud){
                  $adres_nrbud_i = ' AND lokalizacja_nr = "'.$adres_nrbud.'"';
                  }
                      ...
                      echo tabeladb2('15','SELECT * FROM nieruchomosci_lokalowe WHERE id IS NOT NULL'.$grupa_i.''.$ulica_i.''.$adres_nrbud_i.' ORDER BY `id` ASC', '<tr class="text-center">', '</tr>',
                      '<td>', '0', '</td>',
                      '<td class="small">', '1', '</td>',
                      '<td>', '2', '',
                      ' ', '3', '</td>',
                      ...
                      '<td class="text-center dontprint text-nowrap"><a href="nieruchomosci_lokalowe_edytuj.php?id=', '0', '" role="button" class="btn btn-dark btn-sm text-uppercase"><i class="fa fa-pencil" aria-hidden="true"></i> edytuj</a></td>',
      ...
      echo tabeladb2('15','SELECT * FROM nieruchomosci_lokalowe ORDER BY `id` DESC LIMIT 25', '<tr class="text-center">', '</tr>',
      '<td>', '0', '</td>',
      '<td class="small">', '1', '</td>',
      '<td>', '2', '',
      ' ', '3', '</td>',
